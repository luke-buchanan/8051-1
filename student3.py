...
class Student:
    ...
    def __init__(
        ...
        random_state: int = 42,
        *,
        model_type="xgb",  # Options: "ridge", "gbm", "rf", "nn", "lgbm", "xgb"
        ...
        lgbm_objective='huber',        # Huber loss - robust to outliers
        lgbm_alpha=0.9,                # Huber alpha parameter
        ...
        xgb_objective='reg:squarederror',  # Can also use 'reg:pseudohuberror'
        xgb_huber_slope=1.0,           # For pseudohuber loss
        ...
        **kwargs
        ...
        self.lgbm_objective = str(lgbm_objective)
        self.lgbm_alpha = float(lgbm_alpha)
        ...
        self.xgb_objective = str(xgb_objective)
        self.xgb_huber_slope = float(xgb_huber_slope)
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series, meta=None):
        ...
        if self.model_type == "gbm":
            ...
            # LightGBM with Huber loss for robustness
            self.pipe_ = lgb.LGBMRegressor(
                objective=self.lgbm_objective,
                alpha=self.lgbm_alpha,
            ...
            # XGBoost - can use pseudohuber for robustness
            params = {
            ...
            # Add huber_slope only if using pseudohuber objective
            if self.xgb_objective == 'reg:pseudohuberror':
                params['huber_slope'] = self.xgb_huber_slope
            
            self.pipe_ = xgb.XGBRegressor(**params)
            self.pipe_.fit(F.values, y.values)
